      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 bg-slate-900/80 border border-slate-800/80 p-5 rounded-2xl backdrop-blur-xl shadow-xl">
          ...
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight">
                {t('podActivity.title', null, 'POD Activity MQTT Monitor')}
              </h1>
              <span className="text-xs font-mono px-2.5 py-0.5 bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded-full font-bold inline-flex items-center gap-1.5">
                ...
                {isSocketConnected ? t('common.live', null, 'Live Real-Time') : t('common.connecting', null, 'Connecting WebSocket...')}
              </span>
              <span className="text-xs font-mono px-2 py-0.5 bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 rounded-md font-bold">
                {t('podActivity.liveBadge', null, 'Armada V3')}
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-1 flex items-center gap-2 flex-wrap">
              <span>{t('podActivity.topicLabel', null, 'Topik')}: <code className="text-cyan-300 font-mono font-semibold">mod_chair/pob_state</code> | <code className="text-cyan-300 font-mono font-semibold">mod_ambience/pod_state</code></span>
              <span>•</span>
              <span>{t('podActivity.statusLabel', null, 'Status')}: <b className="text-emerald-400 font-mono">1 = {t('podActivity.occupied', null, 'OCCUPIED')}</b>, <b className="text-slate-300 font-mono">0 = {t('podActivity.available', null, 'AVAILABLE (STANDBY)')}</b></span>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-wrap self-end lg:self-center">
          ...
          >
            ...
            <span>{t('podActivity.reconnectMqtt', null, 'Reconnect Broker')}</span>
          </button>
