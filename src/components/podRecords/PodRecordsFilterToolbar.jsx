  ...
  return (
    <div className="px-4 sm:px-6 py-2.5 bg-slate-900/20 border-b border-slate-800/80 flex flex-col gap-2.5 shrink-0">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs font-bold text-slate-300 shrink-0 flex items-center gap-1.5">
            ...
            <span>Pilih Tanggal (InfluxDB):</span>
          </span>

          {availableDates.length === 0 ? (
            <span className="text-xs text-slate-500 italic">Tidak ada rekaman tanggal Influx terdeteksi</span>
          ) : (
            <div className="relative min-w-[240px] sm:min-w-[320px]">
              ...
              >
                {availableDates.map((d) => {
                  ...
                  const countNum = folderMeta?.count ?? folderMeta?.fileCount ?? 0;
                  const sizeInfo = folderMeta?.sizeFormatted || 'Aliran Telemetri';
                  const countStr = folderMeta ? `${countNum} aliran • ${sizeInfo}` : '';
                ...
                {availableDates.length > 1 && (
                  <option value="ALL" className="bg-slate-900 text-cyan-300 py-1 font-sans font-bold">
                    🌐 Semua Aliran Data (Semua Tanggal InfluxDB)
                  </option>
                )}
              </select>
            </div>
      ...
      {activeCategory === 'heartbeats' && (
        <div className={`grid grid-cols-1 sm:grid-cols-2 ${timePreset === 'custom' ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-3 pt-2 border-t border-slate-800/60`}>
          ...
          {/* Custom Time Inputs (Only when timePreset === 'custom') */}
          {timePreset === 'custom' && (
            <div>
