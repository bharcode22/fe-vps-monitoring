...
export default function PodInfluxTemplateExplorerModal({
  isOpen,
  onClose,
  activePod,
  templateSearchTerm = '',
  ...
  onSetDeleteTargetTemplate,
  setDeleteTargetTemplate
}) {
  if (!isOpen) return null;

  const doApplyTemplate = onApplyTemplate || handleApplyTemplate;
  const doOpenCreateTemplate = onOpenCreateTemplate || handleOpenCreateTemplate;
  const doOpenEditTemplate = onOpenEditTemplate || handleOpenEditTemplate;
  const doSetDeleteTargetTemplate = onSetDeleteTargetTemplate || setDeleteTargetTemplate;
  const doSearchTermChange = onTemplateSearchTermChange || setTemplateSearchTerm;
  const doCategoryFilterChange = onTemplateCategoryFilterChange || setTemplateCategoryFilter;

  const templateList = filteredTemplates && filteredTemplates.length > 0
    ? filteredTemplates
    : (templates && filteredTemplates.length === 0 && !templateSearchTerm && templateCategoryFilter === 'Semua' ? templates : filteredTemplates);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 flex-shrink-0">
          <div className="relative flex-1">
            ...
            <input
              ...
              onChange={(e) => doSearchTermChange?.(e.target.value)}
          ...
          >
            {templateCategories.map((cat) => (
              <button
                key={cat}
                type="button"
                onClick={() => doCategoryFilterChange?.(cat)}
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5 self-end sm:self-auto">
                      <button
                        type="button"
                        onClick={() => doApplyTemplate?.(tmpl, false)}
                      ...
                      <button
                        type="button"
                        onClick={() => doApplyTemplate?.(tmpl, true)}
                      ...
                      <button
                        type="button"
                        onClick={() => doOpenEditTemplate?.(tmpl)}
                      ...
                      <button
                        type="button"
                        onClick={() => doSetDeleteTargetTemplate?.({ id: tmpl.id, name: tmpl.name })}
                    ...
                    <div className="flex items-center gap-2 flex-wrap text-[11px] font-mono text-slate-400 pt-1">
                      ...
                      <span className="px-2 py-0.5 bg-slate-900 rounded border border-slate-800 text-slate-300">
                        ...
                        <strong className="text-cyan-400 font-normal">
                          {cfg.measurements && Array.isArray(cfg.measurements)
                            ? cfg.measurements.join(', ')
                            : cfg.measurement || '-'}
                        </strong>
                      </span>
                      <span className="px-2 py-0.5 bg-slate-900 rounded border border-slate-800 text-slate-300">
                        Field:{' '}
                        <strong className="text-teal-300 font-normal">
                          {cfg.fields && Array.isArray(cfg.fields)
                            ? cfg.fields.join(', ')
                            : cfg.field || '-'}
                        </strong>
                      </span>
